# Apresentação do Projeto

## KPI Engenharia — Excel/VBA

Este projeto registra uma aplicação prática de Excel habilitado para macros utilizada para apoiar controles e indicadores operacionais.

### Competências demonstradas

**Excel:** organização de dados, indicadores e estrutura de acompanhamento.

**VBA / automação:** aplicação de lógica para reduzir tarefas manuais e apoiar a rotina operacional.

**Análise:** transformação de dados operacionais em informações mais fáceis de acompanhar.

**Visão de negócio:** conexão entre tecnologia, controles administrativos, engenharia e compras.

Desenvolvi uma ferramenta em Excel/VBA para organizar e acompanhar indicadores operacionais. O projeto surgiu da necessidade de tornar informações de rotina mais estruturadas e fáceis de acompanhar, reduzindo atividades manuais e melhorando a visibilidade para tomada de decisão.


**Projeto Excel/VBA — Indicadores Operacionais**  
Desenvolvimento de ferramenta em Excel habilitada para macros para organização e acompanhamento de KPIs, aplicando automação de rotinas, estruturação de dados e visão gerencial.

## Evoluções possíveis

Uma evolução natural deste projeto é migrar os indicadores para Power BI e automatizar a preparação dos dados com Python/SQL, mantendo o Excel como fonte ou interface operacional.
